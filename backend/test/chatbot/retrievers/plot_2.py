# backend/test/chatbot/retrievers/plot_2.py
...
def retriever_hybrid(book_id: str, query: str, top_k: int = 4) -> List[Document]:
    # load both sources
...
def retriever_by_chapter(book_id: str, intent: str) -> List[Document]:
...
def retriever_by_position(book_id: str, intent: str) -> List[Document]:
    summaries = _load_chapter_summaries(book_id)
    if not summaries:
        return []
...
if __name__ == "__main__":
    ...
    intent = "PLOT_SEMANTIC"
    print(retrieve_plot_context(book_id, question, intent))
